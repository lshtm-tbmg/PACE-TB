    ...
    for (run in 1:n_runs) {
      ...
      for (yr in 1:n_years) {
        if (yr <= 5) {
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * ((1 - (0.04 * yr)) * cost[cost$uid == uid, "c_a_diag_standard"] + (0.04 * yr) * cost[cost$uid == uid, "c_a_diag_improved"]) + # Cost of standard and improved evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DS_standard"] + # Cost of DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"]) * # Cost of DR treatment
            (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * (0.8 * cost[cost$uid == uid, "c_a_diag_standard"] + 0.2 * cost[cost$uid == uid, "c_a_diag_improved"]) + # Cost of standard and improved evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DS_standard"] + # Cost of DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"]) * # Cost of DR treatment
            (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
        }
        ...
        if (yr <= 5) {
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * cost[cost$uid == uid, "c_a_diag_standard"] + # Cost of standard evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DS_standard"] + # Cost of DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"] + # Cost of DR treatment
               (0.04 * yr) * data_temp[["n_diag_noXpert_y"]][yr, run] * cost[cost$uid == uid, "c_a_DST"]) * # Cost of performing DST
            (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * cost[cost$uid == uid, "c_a_diag_standard"] + # Cost of standard evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DS_standard"] + # Cost of DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"] + # Cost of DR treatment
               0.2 * data_temp[["n_diag_noXpert_y"]][yr, run] * cost[cost$uid == uid, "c_a_DST"]) * # Cost of performing DST
            (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
        }
      }
    }
    ...
    for (run in 1:n_runs) {
      ...
      for (yr in 1:n_years) {
        if (yr <= 5) {
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * cost[cost$uid == uid, "c_a_diag_standard"] + # Cost of standard evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * ((1 - (0.04 * yr)) * cost[cost$uid == uid, "c_a_treat_DS_standard"] + (0.04 * yr) * cost[cost$uid == uid, "c_a_treat_DS_improved"]) + # Cost of improved and standard DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"]) # Cost of DR treatment
            (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
          ...
          cea[[iso]][[intv]][["c_Int"]][yr, run] <- 
            (data_temp[["n_assess_y"]][yr, run] * cost[cost$uid == uid, "c_a_diag_standard"] + # Cost of standard evaluation
               data_temp[["n_treat_DS_y"]][yr, run] * (0.8 * cost[cost$uid == uid, "c_a_treat_DS_standard"] + 0.2 * cost[cost$uid == uid, "c_a_treat_DS_improved"]) + # Cost of improved and standard DS treatment
               data_temp[["n_treat_DR_y"]][yr, run] * cost[cost$uid == uid, "c_a_treat_DR_standard"]) # Cost of DR treatment
          (1 + he_temp[["disc_rate"]]["disc", ])^(-(yr - 1)) # Discounting
        }
      }
